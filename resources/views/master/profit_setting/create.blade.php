                ...
                <div class="row">
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <select class="form-control selectModal" id="kode_item" name="kode_item" onchange="selectKodeItem(this.value)">
                                <option value="" selected>Pilih</option>
                                <option value="Semua">Semua</option>
                                @foreach($items AS $itm)
                                    <option value="{{ $itm->id }}">{{ $itm->kode_item }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="col-md-2">
                        ...
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="tipe_profit" id="tipe_profit" value="persentase">
                            <label class="form-check-label" for="tipe_profit">
                            Persentase (%)
                            </label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="tipe_profit" id="tipe_profit" value="nominal">
                            <label class="form-check-label" for="tipe_profit">
                            Nominal (Rp)
                            </label>
                        </div>
...
<script>
    ...
    $("#saveButton").click(function(){
        ...
        $.ajax({
            ...
            url: "{{ url('dataInduk/profitsetting/store') }}",
            data: {
                "kode_item": kode_item,
