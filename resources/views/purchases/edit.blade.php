                        ...
                        <div class="col-sm-6">
                           <div class="form-group">
                              ...
                              <input type="text" class="form-control form-control-sm" name="supplier_code"
                                 id="supplier_code" placeholder="ID Supplier" value="{{$data->supplier_code}}"
                                 autofocus>
                           </div>
                        </div>
                        <div class="col-sm-6">
                           <div class="form-group">
                              ...
                              <input type="text" class="form-control form-control-sm" name="supplier" id="supplier"
                                 placeholder="Nama Supplier" value="{{$data->supplier}}" required>
                           </div>
                        </div>
                     </div>
                     <div class="row">
                           ...
                           <div class="form-group" data-target="#dateSelect" data-toggle="datetimepicker">
                              <div class="input-group date" id="dateSelect" data-target-input="nearest">
                                 ...
                                 <input type="text" class="form-control form-control-sm  datetimepicker-input"
                                    data-target="#dateSelect" min="{{ Date('Y-m-d') }}" value="{{$data->date_order }}"
                     ...
                     <div class="scrollYMenu">
                        <div class="row align-item-center justify-content-around" id="listMenu">
                           ...
                           <div class="col-sm-5 col-md-5 color-palette shadow m-1"
                              ...
                              onclick="addToCart('{{$item->id}}','{{$item->code}}','{{$item->name}}','{{$item->buy_price}}',0)">
               ...
               <div class="card">
                  ...
                  <div class="card-footer pb-0 m-0">
                     <div class="scrollYMenu pr-3" style="height:64vh" id="listCart">
                        {{-- start cart --}}
                        {{-- jquery on action --}}
                        @foreach($purchases_details as $key => $purchases_detail)

                        <div class="card-cart card-cart-{{$key+1}}" id="card-cart-{{$key+1}}"
                           data-id="{{$purchases_detail->code}}" data-item-id="{{$purchases_detail->item_id}}">
                           <div class="card">
                              <div class="card-body">
                                 <input type="hidden" name="purchases_detail_id['{{$key+1}}']"
                                    id="purchases_detail_id_{{$key+1}}" value="{{$purchases_detail->id}}">
                                 <input type="hidden" name="item_id['{{$key+1}}']" id="item_id_{{$key+1}}"
                                    value="{{$purchases_detail->item_id}}">
                                 <input type="hidden" class="" name="sub_total['{{$key+1}}']" id="sub_total_{{$key+1}}"
                                    value="{{$purchases_detail->sub_total}}">
                                 <div class="row align-items-center justify-content-between">
                                    <div class="col-sm-2 col-md-2">
                                       <div class="text-center">
                                          <img src="/dist/img/kopi.png" class="profile-user-img img-fluid img-circle"
                                             alt="nama gambar">
                                       </div>
                                    </div>
                                       ...
                                       <div class="row align-items-center justify-content-between">
                                          <div class="col-md-12 col-md-12">
                                             <p class="h6 py-0 m-0 text-wrap text-uppercase">
                                                {{$purchases_detail->name}} </p>
                                             <input type="hidden" name="buy_price['{{$key+1}}']"
                                                id="buy_price_{{$key+1}}" value="{{$purchases_detail->buy_price}}">
                                             <span class="text-bold text-olive py-0 m-0"
                                                id="buy_price_view_{{$key+1}}">{{Helper::formatNumber($purchases_detail->buy_price,'rupiah')}}</span>
                                          </div>
                                       </div>
                                    </div>
                                          ...
                                          <div class="col-12 ">
                                             <div class="d-flex justify-content-around align-items-stretch">
                                                <button type="button" class="btn btn-danger btn-xs mx-1"
                                                   onClick="adjQty({{$key+1}},'minus')" data-id="{{$key+1}}"> <i
                                                      class="fas fa-minus-circle"></i></button>
                                                <input class="form-control form-control-sm col-sm-5 col-md-5"
                                                   id="quantity{{$key+1}}" name="quantity['{{$key+1}}']"
                                                   value="{{$purchases_detail->quantity}}" readonly required>
                                                <button type="button" class="btn btn-danger btn-xs mx-1"
                                                   onClick="adjQty({{$key+1}},'plus')" data-id="{{$key+1}}"> <i
                                 ...
                                 <div class="row align-items-center justify-content-between">
                                    <div class="col-sm-1 col-md-1">
                                       <button type="button" class="btn btn-outline-danger btn-sm"
                                          onclick="removeCart('card-cart-{{$key+1}}','{{$purchases_detail->id}}')"> <i
                                             class="fas fa-trash"></i></button>
                                    </div>
                                    <div class="col-sm-5 col-md-5">
                                       <input type="text" class="form-control form-control-sm ml-1"
                                          name="catatan['{{$key+1}}']" id="catatan_{{$key+1}}" placeholder="Catatan"
                                          value="{{$purchases_detail->description}}">
                                    </div>
                                    <div class="col-md-6 col-sm-6 align-items-center ">
                                       <span class="h6 text-bold text-olive float-right pl-1"
                                          id="sub_total_view{{$key+1}}">&nbsp;{{Helper::formatNumber($purchases_detail->sub_total,'rupiah')}}</span>
                                    </div>
                                 </div>
                              </div>
                           </div>
...
<script>
   const _renderCard = (id,code,name,buy_price,category_name) =>{
      ...
      card +='onclick="addToCart(\''+id+'\',\''+code+'\',\''+name+'\',\''+buy_price+'\',0)">'
      ...
      card +='</div>'
      card +='</div>'
      // card = '<div class="col-4" onclick="addToCart(\'{{$item->id}}\',\'{{$item->code}}\',\'{{$item->name}}\',\'{{$item->buy_price}}\')"><div class="info-box bg-primary" style="min-height: \'500px\'">';
      ...
      // card+= '<span class="info-box-number">'+buy_price+'</span>';
      // card+= '</div></div></div>';
   ...
   const updateData = (action) => {
         ...
         let href = '/purchase/update/{{$data->code}}/'+action;
            ...
            success: function (res) {
               if(res.status=="success"){
                  swal.fire({
                     ...
                     if (result.isConfirmed) {
                        window.location = "{{route('purchase.edit',$data->code)}}";
                  }
                  })
               }else{
                  Swal.fire(
   ...
   function addToCart(id,code,name,buy_price,qty){
         ...
         cart+= '<span class="text-bold text-olive uang py-0 m-0" id=buy_price_view_'+i+' >'+buy_price+'</span>';
         ...
         cart+= '<input class="form-control form-control-sm col-sm-5 col-md-5" id="quantity'+i+'" name="quantity['+i+']" value="'+qty+'" readonly required>';
         ...
         cart+= '<button type="button" class="btn btn-outline-danger btn-sm" onclick="removeCart(\'card-cart-'+i+'\',\''+id+'\')"> <i class="fas fa-trash"></i></button>';
         cart+= '</div>';
         ...
         if(existCart(i,code)){
            ...
            // jika harga jual mengikuti master item / bukan harga jual dari purchases detail
            // $('#buy_price_'+id).val(buy_price)
            // maskRupiah("#buy_price_view_"+id,buy_price)
            adjQty(id,'plus')
            calcGrand()
